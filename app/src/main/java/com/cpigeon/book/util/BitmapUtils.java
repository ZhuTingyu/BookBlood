...
*/
     ...
     * @param path
     */
    public static boolean saveJPGE_After_PNG(Context context, Bitmap bitmap, String path, int quality) {
        File file = new File(path);
        makeDir(file);
        try {
            FileOutputStream out = new FileOutputStream(file);
...
//        cv.drawBitmap(watermark, w - ww + 5, h - wh + 5, null);// 在src的右下角画入水印
        cv.drawBitmap(watermark, 0, h - wh + 5, null);// 在src的左下角下角画入水印
        // save all clip
...
//        cv.save(Canvas.ALL_SAVE_FLAG);// 保存
     ...
     * @return
     */
    public static Bitmap createBitmapCenter2(Bitmap src, Bitmap watermark) {
        if (src == null) {
            return null;
        }
        int w = src.getWidth();
...
//        cv.save(Canvas.ALL_SAVE_FLAG);// 保存
     ...
     * @return
     */
    public static Bitmap createBitmapLeftTop(Bitmap src, Bitmap watermark3) {
     ...
     * @return
     */
    public static Bitmap createBitmapBottom(Bitmap src, Bitmap watermark3) {

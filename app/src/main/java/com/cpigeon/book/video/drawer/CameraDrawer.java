package com.cpigeon.book.video.drawer;
...
import com.cpigeon.book.video.GetBitmap;
import com.cpigeon.book.video.filter.AFilter;
import com.cpigeon.book.video.filter.CameraFilter;
import com.cpigeon.book.video.filter.GroupFilter;
import com.cpigeon.book.video.filter.NoFilter;
import com.cpigeon.book.video.filter.ProcessFilter;
import com.cpigeon.book.video.filter.WaterMarkFilter;
import com.cpigeon.book.video.gpufilter.filter.MagicBeautyFilter;
import com.cpigeon.book.video.record.video.TextureMovieEncoder;
import com.cpigeon.book.video.utils.EasyGlUtils;
import com.cpigeon.book.video.utils.MatrixUtils;
...
//    private boolean isCameraService = true;//是否有相机服务   true 取判断有无相机选择， false  不去判断
//          if (isCameraService) {
//        if (!cameraIsCanUse()) {
//            return;
//        } else {
//            isCameraService = false;
//        }
//    }

    public GetBitmap getBitmap = new GetBitmap() {
        @Override
        public void setBitmap(Bitmap mBitmap, int tag) {
            try {


                cameraTag = tag;
                if (cameraTag == 2) return;

                if (waterMarkFilter != null) {
                    mBeFilter.removeFilter(waterMarkFilter);
                    waterMarkFilter = new WaterMarkFilter(resources);
                    if (mBitmap != null) {
                        waterMarkFilter.setWaterMark(mBitmap);
//                        waterMarkFilter.setPosition(0, mPreviewHeight - mBitmap.getHeight(), 0, 0);
